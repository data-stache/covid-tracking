...
covid %>%
  select(date, state, new_cases_percap, new_tests_percap, percent_pos, new_death_percap) %>%
  filter(state == st) %>%
  filter(date %in% ind_tywm_date)
...
merge(covid_state_growth, covid_state_zones, by = c("state", "sum_cases_percap", "percent_pos")) %>%
  filter(state == st) %>%
  select(state_name, sum_cases_percap, sum_tests_percap, percent_pos, sum_death_percap, cases_zone, percent_zone)
