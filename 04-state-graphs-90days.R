...
p_new_case <- covid %>%
  ...
  geom_vline(xintercept = ymd(20200925), color = "grey60", size = .2) +
...
p_new_deaths <- covid %>%
  ...
  scale_color_manual(values="light grey") +
  geom_line(aes(y = new_death_07da), size = .25, col="dark red") +
  xlab("Date") +
...
ggsave(paste("figs/", state, sep = " ", "Total Data 90 days.png"),
       G,
       width = p_width,
       height = p_height,
       dpi = "retina")
